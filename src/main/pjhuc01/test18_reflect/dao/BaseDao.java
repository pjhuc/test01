...
import main.pjhuc01.test18_reflect.util.JdbcTemplate;
...
public class BaseDao extends JdbcTemplate {
    ...
    /**
     * 利用反射机制  写一个通用的单个对象的查询方法
     * 要求 javabean 中的字段名和表名一致，javabean中的属性名和字段名一致
     * @param clazz
     * @param empno
     * @return
     */
    public Object get(Class clazz, int empno) {
        String clazzName = clazz.getName();
        System.out.println(clazzName);
        String tableName = clazzName.substring(clazzName.lastIndexOf(".") + 1);
        String lowerTableName = tableName.toLowerCase();
        System.out.println(lowerTableName);
        String sql = "select * from " + lowerTableName + " where "+lowerTableName+"no  = " + empno;
